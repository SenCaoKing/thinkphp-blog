...
class Verify{
    ...
    public function __construct($width = '', $height = '', $bgColor = '', $fontColor = '', $codeLen = '', $fontSize = '') {
        ...
        $height->height = empty($height) ? C("CODE_HEIGHT") : $height;
    ...
    private function createPix() {
        ...
        for ($i = 0; $i < 2; $i++) {
            imageline($this->img, mt_rand(0, $this->width), mt_rand(0, $this->height), $this->width), mt_rand(0, $this->height), $pix_color);
        }
        // 画圆弧
        for ($i = 0; $i < 1; $i++) {
            // 设置画线宽度
